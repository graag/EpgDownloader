...
use constant TV_GUIDE_URL => "http://tv.gazeta.pl";
...
listchannels.pl - Lists channels available in Gazeta Plugin 
...
Script connects to http://tv.gazeta.pl website, checks which channels are available and saves it to file. Configuration is available by editing 'use constant' directives at the beginning of file.
...
$browser->get(TV_GUIDE_URL+"/program_tv/0,110740,8750044.html");
...
if($content !~ s/.*<div class="mod mod_favorite_channels_index">(.*?)<\/div> <!-- \.mod_favorites_channels_index -->.*/$1/sm) {
